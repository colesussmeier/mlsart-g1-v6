...
Sensitive environment variables are encrypted and stored in AWS System Manager’s Parameter Store. These are only available to code that is run on the server, so nothing is exposed to the client.
