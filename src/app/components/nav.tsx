...
const Navbar = () => {
    ...
    return (
        <nav className="flex items-center justify-between flex-wrap bg-white px-6 py-1 lg:py-6">
            <p className="text-3xl max-w-44 lg:max-w-lg">Mary Lou Sussmeier Watercolors</p>
            <div className="block lg:hidden">
                <button onClick={() => setIsOpen(!isOpen)} className="flex items-center px-3 py-2 border rounded border-black hover:text-gray-500 hover:border-gray-500">
                    <svg className="fill-current h-3 w-3" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><title>Menu</title><path d="M0 3h20v2H0V3zm0 6h20v2H0V9zm0 6h20v2H0v15z"/></svg>
                </button>
            </div>
            <div className={`${isOpen ? `block` : `hidden`} w-full block flex-grow text-black lg:flex lg:items-center lg:w-auto lg:justify-end`}>
